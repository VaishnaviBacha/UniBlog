...
import { useGetBlogsByCourseNameQuery } from '@/slices/blogApiSlice';
...
const CourseCategories= () => {
 ...
 const { data: blogsByCourse, isLoading: courseBlogsLoading, refetch: refetchCourse } = useGetBlogsByCourseNameQuery({
    departmentName: selectedDepart,
   ...
   const departmentBlogs: Blog[] = useAppSelector((state)=> state.blog.blogsByDepartment)
    ...
    useEffect(() => {
      ...
      const handleCourseSelect = (event: React.MouseEvent<HTMLDivElement>) => {
        const courseName = event.currentTarget.textContent || ""; // Extract course name from the clicked div
        setSelectedCourse(courseName);
       console.log("corse clicked")
        refetch(); // Assuming this is where you want to trigger a data fetch
      };
    return (
            ...
            {selectedDepartment && (
                <div className="flex  w-full ">
                    {selectedDepartment.courses.map((course) => (
                        <div 
                        key={course.course_id} className="m-2 p-3 rounded-lg bg-slate-400 "
        ...
        {blogsByDepartment && (
             ...
             {selectedCourse && (
        <div className="mt-4">
          <h3 className="text-lg font-semibold mb-2">Blogs in {selectedCourse}</h3>
          {blogsByCourse?.posts?.map((blog) => (
            <div key={blog.id} className="mb-8 border w-full rounded-md p-4 flex">
              <div className="mr-4">
                <img src="/images/blog-image.jpg" alt="Blog Image" className="w-40 h-auto" />
              </div>
